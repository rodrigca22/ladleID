...
def drawBoundingBox(img,bbox,colorBGR=(0,255,0),thickness=2,bias=0):
    cv2.rectangle(img,(bbox[0]-bias, bbox[1]-bias), (bbox[0] + bbox[2]+bias, bbox[1] + bbox[3]+bias), colorBGR, 2)
...
def createHSVTrackbars(windowName='HSVTrackBars',HueMin=0,HueMax=179,SatMin=0,SatMax=255,ValMin=0,ValMax=255):
    ...
    cv2.createTrackbar("Hue Min", windowName, HueMin, 179, empty)
    cv2.createTrackbar("Hue Max", windowName, HueMax, 179, empty)
    cv2.createTrackbar("Sat Min", windowName, SatMin, 255, empty)
    cv2.createTrackbar("Sat Max", windowName, SatMax, 255, empty)
    cv2.createTrackbar("Val Min", windowName, ValMin, 255, empty)
    cv2.createTrackbar("Val Max", windowName, ValMax, 255, empty)
...
def createThresTrackbars(windowName='ThresholdTrackBars',ThresLeft=127,ThresRight=127):
    ...
    cv2.createTrackbar("Thres Left", windowName, ThresLeft, 255, empty)
    cv2.createTrackbar("Thres Right", windowName, ThresRight, 255, empty)
